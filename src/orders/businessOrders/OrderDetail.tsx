...
const { eatout_id, user_id } = JSON.parse(localStorage.getItem('businessInfo')!)
...
const OrderDetail = ({
  ...
  const [cancelUpdateButtonToggle, setCancelUpdateButtonToggle] = useState(
    false,
  )
  ...
  const [canadaPostDeliveryCharges, setCanadaPostDeliveryCharges] = useState('')
  ...
  const updateOrder = async () => {
    ...
    if (calculatingMinimumSpend < minimumSpend) {
      setNotifyMessage(`Your cart minimum spend is ${currency} ${minimumSpend}`)
      setNotifyType('error')
      setNotify(true)
      return
    }
